...
/*
	* Create By Bot
	* Follow https://github.com/Botdev
	* Whatsapp : wa.me/6282113821188
*/

async function GroupUpdate(Bot, update, store) {
	try {
		for (let n of update) {
...
async function GroupParticipantsUpdate(Bot, { id, participants, action }, store) {
	try {
		if (global.db.groups && global.db.groups[id] && global.db.groups[id].welcome && store.groupMetadata && store.groupMetadata[id]) {
			...
			for (let n of participants) {
				let profile;
				try {
					profile = await Bot.profilePictureUrl(n, 'image');
				} catch {
...
async function LoadDataBase(Bot, m) {
	try {
...
async function MessagesUpsert(Bot, message, store) {
	try {
		let botNumber = await Bot.decodeJid(Bot.user.id);
		const msg = message.messages[0];
		if (store.groupMetadata && Object.keys(store.groupMetadata).length === 0) store.groupMetadata = await Bot.groupFetchAllParticipating()
		const type = msg.message ? (getContentType(msg.message) || Object.keys(msg.message)[0]) : '';
		if (!Bot.public && !msg.key.fromMe && message.type === 'notify') return
		if (msg.key.id.startsWith('BAE5')) return
		if (msg.key.id.length === 22) return
		if (!msg.message) return
		const m = await Serialize(Bot, msg, store)
		require('../Bot')(Bot, m, message, store);
		if (type === 'interactiveResponseMessage' && m.quoted && m.quoted.fromMe) {
			...
			apb.key = msg.key
			apb.key.fromMe = areJidsSameUser(m.sender, Bot.user.id);
			...
			Bot.ev.emit('messages.upsert', pbr);
		}
		if (global.db.set && global.db.set[botNumber] && global.db.set[botNumber].readsw) {
			if (msg.key.remoteJid === 'status@broadcast') {
				await Bot.readMessages([msg.key]);
				if (/protocolMessage/i.test(type)) Bot.sendFromOwner(global.owner, 'Status dari @' + msg.key.participant.split('@')[0] + ' Telah dihapus', msg, { mentions: [msg.key.participant] });
				if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
					...
					await Bot.sendFromOwner(global.owner, `Melihat story dari @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
				}
			}
		}
	} catch (e) {
...
async function Solving(Bot, store) {
	Bot.public = true
	
	Bot.serializeM = (m) => MessagesUpsert(Bot, m, store)
	...
	Bot.getName = (jid, withoutContact  = false) => {
		...
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || Bot.groupMetadata(id) || {};
	...
	Bot.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		async function getFileUrl(res, mime) {
			if (mime.includes('gif')) {
				return Bot.sendMessage(jid, { video: res.data, caption: caption, gifPlayback: true, ...options }, { quoted });
			} else if (mime === 'application/pdf') {
				return Bot.sendMessage(jid, { document: res.data, mimetype: 'application/pdf', caption: caption, ...options }, { quoted });
			} else if (mime.includes('image')) {
				return Bot.sendMessage(jid, { image: res.data, caption: caption, ...options }, { quoted });
			} else if (mime.includes('video')) {
				return Bot.sendMessage(jid, { video: res.data, caption: caption, mimetype: 'video/mp4', ...options }, { quoted });
			} else if (mime.includes('audio')) {
				return Bot.sendMessage(jid, { audio: res.data, mimetype: 'audio/mpeg', ...options }, { quoted });
			}
		}
	...
	Bot.sendFakeLink = async (jid, text, title, body, thumbnail, myweb, options = {}) => {
	...
	Bot.sendFromOwner = async (jid, text, quoted, options = {}) => {
		for (const a of jid) {
			await Bot.sendMessage(a.replace(/[^0-9]/g, '') + '@s.whatsapp.net', { text, ...options }, { quoted });
		}
	}
	
	Bot.sendTextMentions = async (jid, text, quoted, options = {}) => Bot.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
	
	Bot.sendAsSticker = async (jid, path, quoted, options = {}) => {
		...
		await Bot.sendMessage(jid, { sticker: { url: result }, ...options }, { quoted });
		return buff;
	}
	...
	Bot.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		const { mime, data, filename } = await Bot.getFile(path, true);
	...
	Bot.sendButtonMsg = async (jid, body = '', footer = '', title = '', media, buttons = [], quoted, options = {}) => {
	...
	Bot.sendCarouselMsg = async (jid, body = '', footer = '', cards = [], options = {}) => {
...
async function Serialize(Bot, m, store) {
	...
	if (m.key) {
		...
		if (m.isGroup) {
			m.metadata = store.groupMetadata[m.chat] || await Bot.groupMetadata(m.chat)
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted.fromMe = m.quoted.sender === Bot.decodeJid(Bot.user.id)
			...
			m.getQuotedObj = async () => {
				...
				return await Serialize(Bot, q, store)
			}
			m.quoted.key = {
				remoteJid: m.msg?.contextInfo?.remoteJid || m.chat,
				participant: m.quoted.sender,
				fromMe: areJidsSameUser(Bot.decodeJid(m.msg?.contextInfo?.participant), Bot.decodeJid(Bot?.user?.id)),
	...
	m.copy = () => Serialize(Bot, proto.WebMessageInfo.fromObject(proto.WebMessageInfo.toObject(m)))
		...
		try {
			if (/^https?:\/\//.test(text)) {
				...
				if (/gif|image|video|audio|pdf/i.test(mime)) {
					return Bot.sendFileUrl(chatId, text, caption, quoted, options)
				} else {
					return Bot.sendMessage(chatId, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
				}
			} else {
				return Bot.sendMessage(chatId, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
			}
